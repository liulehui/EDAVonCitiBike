...
* Shaofei Wang: Interactive Component

* Xuehan Zhao: Visualizations related with usertype and heatmap; Summary


## Description of data
Our data is from [Motivate](https://www.motivateco.com/use-our-data/). Motivate is the most experienced bike share team.They launched CitiBike in 2013 in NYC.            
Besides, they collect [System data](https://www.citibikenyc.com/system-data) as CSV files, which can be downloaded from the link above.   
...
From the above graph boxplot, we see that the outlier such as time duration **below 0** and the outliers **unreasonably large** may cause trouble in our analysis.   
Here, we define that **minute < 0** and **minute > 120** as outliers. From our evaluation, since the ratio of time duration over 120 is about **0.3%**. We can confidently remove those data. Because normally one do not use a bike for that long.  
We then sdraw the distribution after removal of outliers.  
```{r}
...
```{r}
ggplot(data=df,mapping=aes(minute)) + geom_histogram(fill='#fb6a4a',color='black', alpha = 0.8)+ggtitle('Distribution of time duration') +xlab('minutes over one trip') + ylab('Frequency of trips')+ theme_grey(16)
```
...
```{r}
ggplot(df_weather, aes(x=Severe))+
  geom_bar(colour="white", fill="#fb6a4a", alpha = 0.8)+ggtitle("Number of trips per day for severe and unsevere weather condition") +theme_grey(16) + ylab('Frequency')
```
```{r}
dygraph(ts,main = 'Impact of season',ylab = "Frequency") %>% dySeries('..1',label = 'subscriber ') %>% dySeries('..2',label = 'customer') %>% dyLegend(show = "always", hideOnMouseOut = FALSE,width=400) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(3, "Set2"))
